(() => {
  ...
  $("exploreForm").addEventListener("submit",async e=>{
    ...
    showLoading(true);
    try{
      if(mode==="web"){location.href="https://www.google.com/search?q="+encodeURIComponent(query);return;} if(mode==="ai"){location.href="https://www.google.com/aimode?q="+encodeURIComponent(query);return;} if(mode==="break"){location.href="https://www.google.com/search?tbm=isch&q="+encodeURIComponent(query);return;} const r=await fetch("./api/explore",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mode,query})});
